...
# 从本地的txt文件中推理
parser.add_argument('--from_txt2', action='store_true')
parser.add_argument('--src_txt_file', type=str, default='chars.txt')
...
parser.add_argument('--src_font', type=str, default='charset/gbk/FZXKT.TTF')
...
def main():
    ...
    if args.from_txt:
        ...
        with open(args.src_txt_file, 'r', encoding='utf-8') as file:
            src_chars = file.read().splitlines()
        
        img_list = []
        label_list = []
        font = ImageFont.truetype(args.src_font, size=args.char_size)
        for char in src_chars:
            img = draw_single_char(char, font, args.canvas_size)
            tensor_img = transforms.Normalize(0.5, 0.5)(
            ...
            # Assuming you want to collect these for a DataLoader
            img_list.append(tensor_img)
