...
def draw_single_char(ch, font, canvas_size, sub_sample, x_offset=0, y_offset=0):
    ...
    while True:
        ...
        bbox = img.getbbox()
        if bbox is not None:
            break  # 如果 bbox 不为 None，则退出循环
        ch = random.choice(sub_sample)
...
def draw_single_char2(ch, font, canvas_size, x_offset=0, y_offset=0):
    img = Image.new("L", (canvas_size * 2, canvas_size * 2), 0)
    draw = ImageDraw.Draw(img)
    try:
        draw.text((10, 10), ch, 255, font=font)
...
def draw_font2font_example(ch, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes):
    dst_img, char = draw_single_char(ch, dst_font, canvas_size, sub_sample, x_offset, y_offset)
    # check the filter example in the hashes or not
    dst_hash = hash(dst_img.tobytes())
    if dst_hash in filter_hashes:
        return None
    src_img, char2 = draw_single_char(char, src_font, canvas_size, sub_sample, x_offset, y_offset)
...
def draw_font2imgs_example(ch, src_font, dst_img, canvas_size, x_offset, y_offset):
    src_img = draw_single_char2(ch, src_font, canvas_size, x_offset, y_offset)
...
def filter_recurring_hash(charset, font, canvas_size, x_offset, y_offset):
    ...
    sub_sample = _charset[2000:]
    hash_count = collections.defaultdict(int)
    for c in sample:
        img, char4 = draw_single_char(c, font, canvas_size, sub_sample, x_offset, y_offset)
